...
class ProductWooBooking extends StatefulWidget {
  ...
  final Future<Map<String, dynamic>> Function(
      {Map<String, dynamic>? queryParameters, String? endPoint}) getSlots;
...
class _ProductWooBookingState extends State<ProductWooBooking> {
  ...
  DateTime startDate =
      DateTime(nowBookingTime.year, nowBookingTime.month, nowBookingTime.day);
  DateTime endDate = DateTime(
      (nowBookingTime.month >= 11)
          ? nowBookingTime.year + 1
          : nowBookingTime.year,
      (nowBookingTime.month >= 11)
          ? (nowBookingTime.month % 10)
          : nowBookingTime.month + 2);
      ...
      try {
        var data = await widget.getSlots(
          queryParameters: {
            ...
            "max_date": DateFormat("yyyy-MM-dd")
        ...
        if (_pickedTime.value != defaultTime) {
          ...
          widget.onChanged?.call(
              {...?widget.appointment, 'type': WooBookingType.defaultBooking});
        }
  ...
  Future<void> _getAppointmentData() async {
    try {
      ...
      _defaultDateAvailable =
          get(product, ['default_date_availability'], '') == 'available';
  ...
  _handleAvailableMonthsBooking() {
    ...
    if (_defaultDateAvailable) {
      DateTime start =
          getFutureDate(nowBookingTime, _minDateUnit, _minDateValue);
      ...
      case 'month':
        ...
        if (_customDurationValue > _monthsAvailable.length) {
          ...
          if (_customDurationValue > 1) {
            _currentMonthsAvailable = _monthsAvailable.sublist(
                0, _monthsAvailable.length - (_customDurationValue - 1));
          } else {
                              ...
                              children: [
                                ...
                                TableCalendar(
                                  ...
                                  calendarStyle: CalendarStyle(
                                    ...
                                    todayTextStyle: defaultTextCalendar
                                        .copyWith(color: Colors.red),
                                    disabledTextStyle: disableTextCalendar,
                                    selectedTextStyle:
                                        defaultTextCalendar.copyWith(
                                  ...
                                  onDaySelected:
                                    ...
                                    bool customDurationRange =
                                        _customDuration &&
                                            _durationUnit != 'minute' &&
                                            _durationUnit != 'hour';
                                    if (customDurationRange) {
                                      _startDateRange = selectedDay;
                                      switch (_durationUnit) {
                                        case 'day':
                                          _endDateRange = _startDateRange!.add(
                                              Duration(
                                                  days: (_customDurationValue >
                                                          0)
                                                      ? (_customDurationValue *
                                                              _duration) -
                                                          1
