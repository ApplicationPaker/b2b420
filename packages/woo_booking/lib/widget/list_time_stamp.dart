    ...
    if (timeStamps.isNotEmpty && timeStamps.length > 1) {
      List<Widget> listMorning = [
        ...
        ...List.generate(timeStamps.length, (index) {
          if (timeStamps[index].date!.hour >= 0 &&
              timeStamps[index].date!.hour < 12) {
            numMor++;
      ...
      List<Widget> listAfternoon = [
        ...
        ...List.generate(timeStamps.length, (index) {
          if (timeStamps[index].date!.hour >= 12 &&
              timeStamps[index].date!.hour < 17) {
            numAf++;
            return TimeStampItem(
              record: timeStamps[index],
              pickedTime: pickedTime,
      ...
      List<Widget> listEvening = [
        ...
        ...List.generate(timeStamps.length, (index) {
          if (timeStamps[index].date!.hour >= 17 &&
              timeStamps[index].date!.hour < 24) {
            numEv++;
            return TimeStampItem(
              record: timeStamps[index],
              pickedTime: pickedTime,
