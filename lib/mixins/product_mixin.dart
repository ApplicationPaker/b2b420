...
mixin ProductMixin {
  ...
  Widget buildTagExtra(
    ...
    if (enableSale! && product.onSale!) {
      if (product.type == productTypeVariable ||
        ...
        String text =
            translate('sale_percent', {'percent': '${percent.floor()}'});
        children.add(
          ui.Badge(
            text: Text(
              text,
  ...
  Widget buildAddCart(
    ...
    Icon iconWidget = Icon(icon ?? FeatherIcons.plus, size: 34);
    ...
    Widget button = SizedBox(
      height: 40,
      width: titleWidget == null ? 40 : null,
  ...
  Widget buildBrand(
    ...
    String name = brands
        .map((ProductBrand? brand) => brand?.name ?? '')
        .toList()
        .join(' | ');
    return Text(name,
        style: Theme.of(context).textTheme.bodySmall?.copyWith(color: color));
