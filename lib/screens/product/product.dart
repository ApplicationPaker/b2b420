...
class _ProductScreenState extends State<ProductScreen>
  ...
  void didChangeDependencies() async {
    ...
    if (widget.args != null && widget.args?['popup'] != null) {
      _popup = true;
    }
  ...
  Future<void> _handleAddToCart([bool? goCartPage]) async {
    ...
    try {
      ...
      if (_product!.type == productTypeAppointment) {
        ...
        await _cartStore
            .addToCart({'id': _product!.id, 'quantity': _qty, ..._preAddons()});
      }
      if (mounted) {
        showSuccess(
          context,
  ...
  Widget build(BuildContext context) {
    ...
    return Observer(builder: (_) {
      ...
      bool enableAppbar = get(data.configs, ['enableAppbar'], false);
      ...
      var image = rowList.last;
      rowList.removeLast();
      rowList.insert(0, image);
  ...
  List<Widget> buildColumn(List<dynamic>? columns,
      {String? themeModeKey, String? languageKey}) {
    ...
    return columns.map((e) {
      ...
      Color foreground = ConvertData.fromRGBA(
          get(e, ['value', 'foreground', themeModeKey]),
          Theme.of(context).scaffoldBackgroundColor);

      String textHtml = get(e, ['value', 'textHtml', languageKey], '');
      String typeStatus = get(e, ['value', 'typeStatus'], 'text');

      String url = get(e, ['value', 'url', languageKey], '');

      double? height =
          ConvertData.stringToDouble(get(e, ['value', 'height'], '200'));

      bool syncAuth = get(e, ['value', 'syncAuth'], false);
      String thumbSize = get(e, ['value', 'thumbSize'], 'src');
  ...
  Widget buildBlock(
    ...
    switch (type) {
      ...
      case ProductBlocks.featuredImage:
        return _CustomImagePageView(product: _product!);
      ...
      case ProductBlocks.sortDescription:
        if (_product?.shortDescription?.isNotEmpty ?? false) {
      ...
      case ProductBlocks.additionInformation:
        if (_product?.attributes?.isNotEmpty ?? false) {
...
class _CustomImagePageView extends StatefulWidget {
  final Product product;

  const _CustomImagePageView({required this.product});

  @override
  State<_CustomImagePageView> createState() => _CustomImagePageViewState();
}

class _CustomImagePageViewState extends State<_CustomImagePageView> {
  ...
  @override
  Widget build(BuildContext context) {
    bool existThisImage(int targetIndex) {
      return targetIndex + 1 <= (widget.product.images?.length ?? 0) &&
          targetIndex > -1;
    }
      ...
      child: SizedBox(
        ...
        child: Stack(
          ...
          children: [
            ...
            if (existThisImage(activeIndex - 1))
              Positioned(
                right: 10,
                child: GestureDetector(
                  ...
                  child: const Icon(
                    ...
                    color: Color.fromRGBO(237, 89, 38, 1),
                  ),
                ),
              ),
            if (existThisImage(activeIndex + 1))
