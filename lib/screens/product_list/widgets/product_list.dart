...
class _ProductListLayoutState extends State<ProductListLayout>
  ...
  Widget build(BuildContext context) {
    ...
    Map settingItem = itemSettingList.elementAt(widget.typeView) ??
        itemSettingList.elementAt(0);
    String? layout = get(settingItem, ['layout'], 'grid');
    String? type = get(settingItem, ['type'], Strings.productItemContained);
    if (layout == 'list') {
      type = Strings.productItemHorizontalNew;
    }
    if (layout == 'grid') {
      type = Strings.productItemGridNew;
    }
    print('layout: $layout');
    print('type: $type');
              ...
              (_, index) {
                return LayoutBuilder(
                  builder: (_, BoxConstraints constraints) {
                    ...
                    double heightImage =
                        (widthImage * size!.height) / size.width;
