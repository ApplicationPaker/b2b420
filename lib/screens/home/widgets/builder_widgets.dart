...
class BuilderWidgets extends StatefulWidget {
  const BuilderWidgets(
      {Key? key,
      this.hasInnerAppBar = true,
      this.data,
      this.appBar,
      this.extraData,
      this.footer,
      this.appBarType = 'floating'})
      : super(key: key);

  final Data? data;
  final Map? extraData;
  final bool hasInnerAppBar;
  final Widget? footer;
  final Widget? appBar;
...
class _BuilderWidgetsState extends State<BuilderWidgets>
  ...
  Widget build(BuildContext context) {
    ...
    // Appbar
    String? appbarType = get(data.configs, ['appBarType'], widget.appBarType);
    ...
    return Scaffold(
      ...
      body: enableStackLayout && widgetIds.isNotEmpty
          ? buildStack(
              hasInnerAppBar: widget.hasInnerAppBar,
            ...
            child: DraggableScrollableSheet(
              ...
              builder: (
                ...
                return buildScroll(
                    ...
                    footer: footer,
                    hasInnerAppBar: hasInnerAppBar ?? true,
  ...
  Widget buildScroll(
    ...
    return CustomScrollView(
      ...
      controller: scroll,
      slivers: <Widget>[
        if (!isAppbar && enableAppbar! && hasInnerAppBar) appbar,
        if (isRefresh)
        ...
        buildWidgets(context,
            widgetIds: widgetIds, widgets: widgets, footer: footer),
      ],
    );
  }
}
