    ...
    return Observer(
      builder: (_) {
        ...
        return Scaffold(
          ...
          body: loading
              ? buildLoading(context, isLoading: true)
              : AddressChild(
                  prevAddress: getAddress(customer),
...
class AddressChild extends StatefulWidget {
  final Map<String, dynamic>? prevAddress;
...
class _AddressChildState extends State<AddressChild> with LoadingMixin {
  ...
  void didChangeDependencies() {
    if (widget.prevAddress != null) {
      setState(() {
        data = widget.prevAddress!;
      });
    }
  ...
  Widget buildForm() {
    ...
    Map<String, List<CountryAddressData>>? states =
        widget.countryType == 'shipping'
            ? widget.addressDataStore!.address?.shippingStates
            : widget.addressDataStore!.address?.billingStates;
    // data['country'] = 'ישראל';
    widget.addressFields.remove('billing_country');
    widget.addressFields.remove('billing_postcode');
