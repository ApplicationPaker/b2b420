...
class _ProductCategoryWidgetState extends State<ProductCategoryWidget>
  ...
  Widget build(BuildContext context) {
    ...
    int limit = ConvertData.stringToInt(get(fields, ['limit'], 8));
    ...
    return Observer(
      builder: (_) {
        ...
        List<ProductCategory?> categories = categoriesList;
  ...
  Widget buildLayout(
    ...
    switch (layout) {
      ...
      case Strings.productCategoryLayoutGridWrapAtFirst:
        int column = ConvertData.stringToInt(get(styles, ['col'], 2), 2);
        double? ratio =
            ConvertData.stringToDouble(get(styles, ['ratio'], 1), 1);
        ...
        List<ProductCategory?> remainCategories = categories
                ?.where((element) => !mainCategories.contains(element))
                .toList() ??
            [];
          ...
          child: Column(
            children: [
              Wrap(
                ...
                children: List.generate(remainCategories.length, (index) {
                  Map<String, dynamic>? targetTemplateForWrap = {...?template};
                  targetTemplateForWrap['template'] = 'no-image';

                  return buildWrapItem(
                    category: remainCategories[index],
                    styles: styles,
                    template: targetTemplateForWrap,
                    width: 100,
                    height: 100,
                  );
                }),
