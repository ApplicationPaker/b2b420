...
class _ProductVideoShopWidgetState extends State<ProductVideoShopWidget> with Utility, PostMixin {
  ...
  Future<void> _loadMore(List<Product> products) async {
    _productsStore!.getProducts(
      customQuery: {
        ...
        'exclude': [..._watchedVideoStored, ...products.map((e) => e.id.toString()).toList(),
          ...exProducts.map((e) => e.id.toString()).toList()].join(','),
        'page': 1,
        'per_page': _limit,
      },
    ).then((value) {
